...
#读入训练集
with open('./dataset/train/seq.in', encoding='utf-8') as ftrain_feature:
    train_feature_line = [line.strip() for line in ftrain_feature.readlines()]
with open('./dataset/train/seq.out', encoding='utf-8') as ftrain_label:
    train_label_line = [line.strip() for line in ftrain_label.readlines()]
#读入验证集
with open('./dataset/test/seq.in', encoding='utf-8') as ftest_feature:
    test_feature_line = [line.strip() for line in ftest_feature.readlines()]
with open('./dataset/test/seq.out', encoding='utf-8') as ftest_label:
...
w2v_model = gensim.models.KeyedVectors.load_word2vec_format(word2vec_path,binary=False, encoding='utf-8')
...
word2id['[PAD]'] = len(word2id)                                       # id -> word
...
# for feature,label,seq_len in train_dataloader:
